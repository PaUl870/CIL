...
class SimpleGCN(nn.Module):
    def __init__(self, num_users, num_items, embedding_dim, num_layers=3, layer_type="LightGCN", dropout=0.1, max_degree=20, sample_strategy="extreme", extreme_alpha=2.0):
        ...
        if layer_type == "GCN":
            self.layers = nn.ModuleList([GCNLayer.GCNLayer(emb_dim=embedding_dim, dropout=self.dropout) for _ in range(num_layers)])
    ...
    def forward(self, user_indices, item_indices, edge_index, weights):
        ...
        for layer in self.layers:
            ...
            u_emb = u_emb - u_emb.mean(dim=0, keepdim=True)
            i_emb = i_emb - i_emb.mean(dim=0, keepdim=True)
        ...
        u_final = self.project(u_final)
        i_final = self.project(i_final)

        u_init = self.user_embedding.weight[user_indices]
        i_init = self.item_embedding.weight[item_indices]
        prediction = torch.sigmoid((u_final * i_final).sum(dim=-1)/math.sqrt(self.embedding_dim))
